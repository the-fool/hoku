...
class LedTCPServer:
    def __init__(self, mono_seq_obs, fadecandy_host='127.0.0.1', num_steps=16):

        self.client = OpcClient('{}:7890'.format(fadecandy_host))
        self.num_steps = num_steps
        # Test if it can connect (optional)
        if self.client.can_connect():
            print('connected to %s' % fadecandy_host)
        else:
            # We could exit here, but instead let's just print a warning
            # and then keep trying to send pixels in case the server
            # appears later
            print('WARNING: could not connect to %s' % fadecandy_host)
