...
var ws = new WebSocket('ws://' + hn + ':7700');
...
function drawKind() {
  ...
  var el = void 0;
  if (Bug.kind === 0) el = kindEl.append('rect').attr('width', '30').attr('height', '30');
  if (Bug.kind === 1) el = kindEl.append('circle').attr('r', '15');
...
function gotoGameState() {
  ...
  timer = d3.timer(function (elapsed) {
    if (elapsed > length) {
      ...
      var newDegree = elapsed / length * 360;
...
function drawController() {
  var bigButton = controllerEl.append('circle').attrs({
    id: 'ding-button',
    cx: 50,
    cy: 50,
    r: 30,
  ...
  var dieButton = controllerEl.append('circle').attrs({
    id: 'die-button',
    cx: 50,
    cy: 130,
    r: 15,
  ...
  var radialTimer = controllerEl.append('path').attrs({
    id: 'radial-path',
...
function setup() {
  ...
  pitchEl = controlsEl.append('g').attr('id', 'pitch').attr('transform', 'translate(80 15)');

  deployEl = controlsEl.append('g').attr('id', 'deploy').attr('transform', 'translate(220, 15)');

  var bv = d3.select('#bug-viewer');
  var bvBox = bv.node().getBoundingClientRect();
  bugViewerEl = bv.select('svg').attr('height', bvBox.width - 30).attr('width', bvBox.width - 30);
...
function verticalize() {
  $('.pitch-box').each(function () {
    ...
    text.split('').forEach(function (letter) {
      return x.append('<div>' + letter + '</div>');
    });
  });
}
$(function () {
